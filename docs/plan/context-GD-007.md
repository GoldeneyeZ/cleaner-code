...
- `cleaner-code-skillsets/auditors/*/SKILL.md`
- `docs/plan/goal-progression.md`
- `docs/plan/context-GD-007.md`

## Handoff Notes

The next loop should read `docs/plan/goal-progression.md`, select GD-008, and create `cleaner-code-skillsets/conventions/clean-code-conventions/SKILL.md`.
...
- File count:
  `find cleaner-code-skillsets/auditors -mindepth 2 -maxdepth 2 -name SKILL.md -type f | sort | wc -l`
  returned `16`.
- Frontmatter:
  `rg -n "^name: |^description: Use when" cleaner-code-skillsets/auditors/*/SKILL.md`
  found `name` and trigger-oriented `description` entries for all auditor skills.
- Findings-first and severity rubric:
  `rg -n "audit-severity-rubric|Findings first|ordered by severity|file and line references" cleaner-code-skillsets/auditors/*/SKILL.md`
